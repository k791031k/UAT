javascript:(function() {
  ...
  /**
   * ===================================================================
   * 商品查詢小工具 v18.0.0 (最終整合版)
   *
   * @version 18.0.0
   * @description
   * - [架構] 新增「通路查詢」的反向載入邏輯，與其他查詢模式流程區分。
   * - [功能] 實作 POLPLN 的加工計算邏輯。
   * - [功能] 新增點擊單筆資料列即可重新載入該筆細節的功能。
   * - [功能] 加回 Token 設定中的「略過」按鈕與其完整功能。
   * - [UI] 優化「銷售通路」欄位的呈現方式，包含顏色、排序與懸停提示。
   * ===================================================================
   */
  ...
  const ConfigModule = Object.freeze({
    ...
    VERSION: '18.0.0-Final-Integration',
    QUERY_MODES: {
      PLAN_CODE: 'planCode',
  ...
  const UIModule = (() => {
    const injectStyle = () => {
      ...
      s.textContent = `
        ...
        .pct-btn{display:inline-flex;align-items:center;justify-content:center;margin:0;padding:8px 18px;font-size:15px;border-radius:var(--border-radius-base);border:1px solid transparent;background:var(--primary-color);color:#fff;cursor:pointer;transition:background var(--transition-speed),transform var(--transition-speed),box-shadow var(--transition-speed), color var(--transition-speed), border-color var(--transition-speed);font-weight:600;box-shadow:0 2px 5px var(--box-shadow-light);white-space:nowrap;}
        ...
        .pct-table th:nth-child(1) { width: 6.8%; }
        .pct-table th:nth-child(2) { width: 6.8%; }
        .pct-table th:nth-child(3) { width: 19.3%; }
        .pct-table th:nth-child(4) { width: 5.7%; }
        .pct-table th:nth-child(5) { width: 4.5%; }
        .pct-table th:nth-child(6) { width: 4.5%; }
        .pct-table th:nth-child(7) { width: 11.4%; }
        .pct-table th:nth-child(8) { width: 11.4%; }
        .pct-table th:nth-child(9) { width: 6.8%; }
        ...
        .pct-table tr.clickable:hover { background: #e3f2fd; cursor: pointer; } /* For single row refetch */
        .pct-channel-insale { color: var(--primary-color); }
        .pct-channel-offsale { color: var(--error-color); }
        .pct-channel-insale + .pct-channel-insale, .pct-channel-offsale + .pct-channel-offsale { margin-left: 8px; }
        .filter-controls{display:flex;align-items:center;gap:16px;margin-bottom:16px;}
        .search-container{flex:1 1 300px;position:relative;}
        .search-input{width:100%;font-size:14px;padding:8px 12px;border-radius:5px;border:1px solid var(--border-color);box-sizing:border-box;}
        .status-filters{display:flex;gap:10px;}
        .status-filter-btn{font-size:13px;padding:5px 12px !important;flex-shrink:0;}
        ...
        .summary{font-size:15px;margin-bottom:10px;display:flex;align-items:center;gap:15px;flex-wrap:wrap;}
        .summary b{color:var(--warning-color);}
  ...
  const DataModule = (() => {
        ...
        const groupedByPlanCode = rawData.reduce((acc, cur) => {
            if (!acc[cur.planCode]) {
                acc[cur.planCode] = {
                    ...
                    _isErrorRow: false, _loadingProduct: true
                };
            }
    ...
    const getProductDetails = async (planCode) => {
        const { cacheProduct } = StateModule.get();
        if (cacheProduct.has(planCode)) return cacheProduct.get(planCode);
        const res = await ApiModule.callApi('/planCodeController/query', { planCode: planCode, currentPage: 1, pageSize: 1 });
    ...
    const getPolplnData = async (planCode) => {
        const { cacheDetail } = StateModule.get();
        if (cacheDetail.has(planCode)) return cacheDetail.get(planCode);
        const detail = await ApiModule.callApi('/planCodeController/queryDetail', { planCode, currentPage: 1, pageSize: 50 });
    ...
    const getChannelDetails = async (planCode) => {
        const { cacheChannel } = StateModule.get();
        if (cacheChannel.has(planCode)) return cacheChannel.get(planCode);
        const sale = await ApiModule.callApi('/planCodeSaleDateController/query', { planCode, currentPage: 1, pageSize: 50 });
        const channels = (sale.planCodeSaleDates?.records || []).map(r => ({ channel: UtilsModule.channelAPIToUI(r.channel), saleStartDate: UtilsModule.formatDateForUI(r.saleStartDate), saleEndDate: UtilsModule.formatDateForUI(r.saleEndDate), rawStart: r.saleStartDate, rawEnd: r.saleEndDate }));
        cacheChannel.set(planCode, channels);
        return channels;
    };

    const loadDetailsForSingleRow = async (item) => {
        const today = UtilsModule.formatToday();
        const [polpln, channels] = await Promise.all([getPolplnData(item.planCode), getChannelDetails(item.planCode)]);
        ...
        ControllerModule.updateSingleRowByPlanCode(item.planCode);
    };

    const loadProductForSingleRow = async (item) => {
        const today = UtilsModule.formatToday();
        const productData = await getProductDetails(item.planCode);
        if (productData) {
            Object.assign(item, {
        ...
        await loadDetailsForSingleRow(item); // Also load POLPLN
        ControllerModule.updateSingleRowByPlanCode(item.planCode);
    };
    ...
    return {
        queryMultiplePlanCodes, queryChannelData, processRawDataForTable, processChannelDataForTable,
        loadDetailsForSingleRow, loadProductForSingleRow, sortData
    };
  })();
  ...
  const ControllerModule = (() => {
    ...
    const showQueryDialog = () => {
        ...
        `, modal => {
            ...
            const resetLocalState = () => { localState = { mode: '', input: '', masterStatus: new Set(), channelStatus: '', channels: new Set() }; updateUI(); };
            const updateUI = () => {
            ...
            modal.querySelector('#pct-mode-wrap').onclick = e => { const card = e.target.closest('.pct-mode-card'); if (card) { localState = { ...localState, mode: card.dataset.mode }; updateUI(); } };
            ...
            modal.querySelector('#pct-clear-reset').onclick = resetLocalState;
            okBtn.onclick = () => {
                ...
                if (localState.mode === ConfigModule.QUERY_MODES.MASTER_CLASSIFIED) { if (localState.masterStatus.size === 0) { UIModule.showError('請至少選擇一個商品銷售時間', 'pct-query-err'); return; } }
    ...
    const executeQuery = async () => {
        ...
        try {
            ...
            if (queryMode === ConfigModule.QUERY_MODES.CHANNEL_CLASSIFIED) {
                ...
                detailLoader = () => Promise.all(processedData.map(item => DataModule.loadProductForSingleRow(item)));
            } else {
                // 原有流程
                ...
                detailLoader = () => Promise.all(processedData.filter(item => item._loadingDetails).map(item => DataModule.loadDetailsForSingleRow(item)));
            }
    ...
    const handleRowClick = async (planCode) => {
        ...
        if (item._loadingProduct) {
            ...
            UIModule.Toast.show(`重新查詢 ${planCode} 的POLPLN與通路...`, 'info');
    ...
    const renderTable = () => {
        ...
        const headers = [
            { key: 'no', label: 'No' }, { key: 'planCode', label: '代號' }, { key: 'shortName', label: '名稱' }, { key: 'currency', label: '幣別' }, { key: 'unit', label: '單位' }, { key: 'coverageType', label: '類型' }, { key: 'saleStartDate', label: '主約銷售日' }, { key: 'saleEndDate', label: '主約停賣日' }, { key: 'mainStatus', label: '險種狀態' }, { key: 'polpln', label: '商品名稱' }, { key: 'channels', label: '銷售通路' }
        ];
            ...
            <div class="pct-modal-body">
                ...
                <div class="pct-table-wrap"><table class="pct-table">
                    ...
                    <tbody>${pageData.map((r, index) => {
                        ...
                        const inSaleChannels = (r.channels || []).filter(c => c.status === ConfigModule.MASTER_STATUS_TYPES.IN_SALE).sort((a,b) => a.channel.localeCompare(b.channel));
                        const offSaleChannels = (r.channels || []).filter(c => c.status !== ConfigModule.MASTER_STATUS_TYPES.IN_SALE).sort((a,b) => a.channel.localeCompare(b.channel));
                        const channelHTML = [
                            ...inSaleChannels.map(c => `<span class="pct-channel-insale" title="${c.channel} (${c.status}): ${c.saleStartDate}-${c.saleEndDate}">${c.channel}</span>`),
                            ...(inSaleChannels.length > 0 && offSaleChannels.length > 0 ? ['|'] : []),
                            ...offSaleChannels.map(c => `<span class="pct-channel-offsale" title="${c.channel} (${c.status}): ${c.saleStartDate}-${c.saleEndDate}">${c.channel}</span>`)
                        ].join('&nbsp;');
                        ...
                        return `<tr data-plan-code="${r.planCode}" ${specialAttr}>
                            ...
                            <td data-raw="${(r.channels||[]).map(c=>`${c.channel}:${c.saleEndDate}`).join('; ')}">${channelHTML || '...'}</td>
                        </tr>`;
                    }).join('')}</tbody>
                </table></div>
            </div>
        ...
        UIModule.Modal.show(modalHTML, modal => {
            ...
            modal.querySelectorAll('.pct-table tbody tr').forEach(tr => { if(tr.classList.contains('clickable')) { tr.onclick = () => handleRowClick(tr.dataset.planCode); } });
    ...
    const updateSingleRowByPlanCode = (planCode) => {
        ...
        renderTable(); // Easiest way to reflect all changes is to re-render
    };
