javascript:(function() {
  ...
  /**
   * ===================================================================
   * 商品查詢小工具 v13.0.0 (最終決策版)
   *
   * @version 13.0.0
   * @description
   * - [流程變更] 依使用者決策，啟動後一律進入查詢主畫面，不再進行前置 Token 驗證。
   * - [功能修正] 依使用者決策，移除 Token 輸入框的自動轉大寫功能，以確保金鑰正確性。
   * - [功能增加] 自動讀取 Token 的來源增加 'euisToken' 選項。
   * ===================================================================
   */
   ...
   * @description 全域狀態管理，存放應用程式在執行期間的動態資料。
   * ========================================================
   */
  const StateModule = (() => {
    const state = {
   ...
   * @description 負責所有 DOM 操作、樣式注入、介面渲染。
   * ========================================================
   */
  const UIModule = (() => {
    const injectStyle = () => {
      ...
      s.textContent = `
        ...
        .pct-table th, .pct-table td{border:1px solid #ddd;padding:8px 10px;vertical-align:top;word-wrap:break-word;}
        .pct-table th { background:#f8f8f8;position:sticky;top:0;z-index:1;cursor:pointer;text-align:center;}
        .pct-table th:nth-child(1) { width: 50px; }
        .pct-table th:nth-child(2) { width: 90px; }
        .pct-table th:nth-child(3) { width: 250px; }
        .pct-table th:nth-child(4), .pct-table th:nth-child(5), .pct-table th:nth-child(6) { width: 60px; }
        .pct-table th:nth-child(7), .pct-table th:nth-child(8) { width: 95px; }
        .pct-table th:nth-child(9) { width: 90px; }
        .pct-table th:nth-child(10) { width: 120px; }
        .pct-table th:nth-child(11) { width: auto; }
   ...
   * @description 集中處理全域事件監聽和通用事件處理器。
   * ========================================================
   */
  const EventModule = (() => {
  ...
  const ApiModule = (() => {
    ...
    // 此函式在 v13.0.0+ 的流程中不再使用，但為保持程式碼完整性而保留。
   ...
   * @description 負責複雜的資料查詢、處理、轉換與排序。
   * ========================================================
   */
  const DataModule = (() => {
   ...
   * @description 應用程式的指揮中心，協調各模組完成業務邏輯。
   * ========================================================
   */
  const ControllerModule = (() => {
    const initialize = async () => {
      ...
      // [流程變更] 啟動時一律進入查詢主畫面，不再進行前置Token驗證
      // [功能增加] 增加 euisToken 作為 Token 讀取來源
      const storedToken = [
    ...
    const showTokenDialog = () => {
      ...
      `, modal => {
        ...
        // [功能修正] 移除Token輸入框的自動轉大寫功能，因其會造成金鑰錯誤。
        // tokenInput.addEventListener('input', EventModule.autoFormatInput); // This line is removed.

        modal.querySelector('#pct-token-save').onclick = () => {
            const val = tokenInput.value.trim();
            localStorage.setItem('SSO-TOKEN', val); // 統一儲存為 SSO-TOKEN
            if (!val) {
        ...
        const modalHTML = `
            ...
            <div class="pct-modal-body">
                ...
                <div class="pct-table-wrap"><table class="pct-table">
                    ...
                    <tbody>${pageData.map(r=>r._isErrorRow?`<tr class="error-row"><td colspan=11>代號 ${UtilsModule.escapeHtml(r.planCode)} 查詢失敗</td></tr>`
                        :`<tr${r.specialReason?` class="special-row" title="${UtilsModule.escapeHtml(r.specialReason)}"`:''}>
                        ${headers.slice(0,-1).map(h=>`<td data-raw="${UtilsModule.escapeHtml(r[h.key]||'')}">${UtilsModule.escapeHtml(r[h.key]||'')}</td>`).join('')}
                        <td data-raw="${(r.channels||[]).map(c=>`${c.channel}:${c.saleEndDate}`).join('; ')}">${(r.channels||[]).map(c=>`<span>${UtilsModule.escapeHtml(c.channel)}:${UtilsModule.escapeHtml(c.saleEndDate)}</span>`).join('<br>')}</td></tr>`
                    ).join('')}</tbody>
                </table></div>
            </div>
            <div class="pct-modal-footer">
