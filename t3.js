javascript:(function() {
  ...
  const ConfigModule = Object.freeze({
    ...
    VERSION: '20.0.0-ArchitectRefactor',
    QUERY_MODES: {
      PLAN_CODE: 'planCode',
  ...
  /**
   * 狀態管理模組
   * 全局狀態的單一來源 (Single Source of Truth)。
   * 新架構核心：productMap 和 channelMap 用於儲存預載的索引資料。
   */
  const StateModule = (() => {
    const state = {
      ...
      // --- 新增：索引地圖 ---
      productMap: new Map(), // 儲存所有商品主檔資料，以 planCode 為 key
      channelMap: new Map(), // 儲存所有通路資料，以 planCode 為 key
      ...
      // --- 快取 ---
      cacheDetail: new Map(), // POLPLN 的快取
  ...
  /**
   * UI 介面管理模組
   * 負責所有 DOM 操作、樣式注入、UI 元件（Modal, Toast）的管理。
   * 新增：createElement 輔助函式，減少 innerHTML 的使用。
   * 修改：CSS 中新增 .pct-view-results .pct-modal-header 規則，修正標題列間距。
   */
  const UIModule = (() => {
    // 新增：DOM 元素建立輔助函式
    ...
    const injectStyle = () => {
      const styleContent = `
        :root{
          ...
          --secondary-dark-color:#5A6268; --success-color:#5CB85C; --success-dark-color:#4CAE4C;
          --error-color:#D9534F; --error-dark-color:#C9302C; --warning-color:#F0AD4E;
          --warning-dark-color:#EC971F; --info-color:#5BC0DE; --info-dark-color:#46B8DA;
          --background-light:#F8F8F8; --surface-color:#FFFFFF; --border-color:#E0E0E0;
          --text-color-dark:#1a1a1a; --text-color-light:#333333; --box-shadow-light:rgba(0,0,0,0.08);
          --box-shadow-medium:rgba(0,0,0,0.15); --box-shadow-strong:rgba(0,0,0,0.3);
          --border-radius-base:6px; --border-radius-lg:10px; --transition-speed:0.25s;
        }
        ...
        .pct-modal[data-size="query"] { width: 520px; height: auto; max-height: 90vh; }
        .pct-modal[data-size="results"] { width: 1050px; height: 80vh; max-height: 800px; }
        ...
        /* 【UI修正】針對結果頁的 Header，使用較小的垂直 padding，解決標題列間距過高問題 */
        ...
        .pct-confirm-toast{display:flex; align-items:center; gap:15px;}
        .pct-confirm-toast .pct-btn{padding:5px 10px; font-size:14px;}
    ...
    const Toast = {
      show: (msg, type = 'info', duration = 3000) => {
        ...
        if (e) e.remove();
        e = createElement('div', { id: 'pct-toast', className: `pct-toast ${type}`, textContent: msg });
        document.body.appendChild(e);
      ...
      confirm: (msg, onConfirm) => {
        ...
        toastEl.innerHTML = ''; // Clear previous content
        toastEl.appendChild(createElement('div', {
          className: 'pct-confirm-toast',
          children: [
            createElement('span', { textContent: msg }),
            createElement('button', { id: 'pct-confirm-ok', className: 'pct-btn', textContent: '確認', onclick: () => { Toast.close(); onConfirm(true); }}),
            createElement('button', { id: 'pct-confirm-cancel', className: 'pct-btn pct-btn-secondary', textContent: '取消', onclick: () => { Toast.close(); onConfirm(false); }})
          ]
        }));
      },
    ...
    const Modal = {
      ...
      show: (modalContent, onOpen) => {
        Modal.close();
        const mask = createElement('div', { id: 'pctModalMask', className: 'pct-modal-mask' });
        document.body.appendChild(mask);
        
        const modal = createElement('div', { id: ConfigModule.TOOL_ID, className: 'pct-modal', children: modalContent });
    ...
    const Progress = {
        show: (text) => {
            ...
            p = createElement('div', {
                id: 'pct-progress-container',
                className: 'pct-progress-container',
                children: [
                    createElement('span', { className: 'pct-progress-text', textContent: text }),
                    createElement('div', { className: 'pct-progress-bar-wrapper', children: [
                        createElement('div', { id: 'pct-progress-bar', className: 'pct-progress-bar' })
                    ]}),
                    createElement('button', { id: 'pct-abort-btn', className: 'pct-btn pct-abort-btn', textContent: '中止', onclick: () => {
                        StateModule.get().currentQueryController?.abort();
                        Progress.hide();
                        Toast.show('操作已中止', 'warning');
                    }})
                ]
            });
            anchor.prepend(p);
    ...
    return { injectStyle, Toast, Modal, Progress, showError, hideError, createElement };
  })();
  ...
  /**
   * 資料處理模組
   * 新架構核心：負責預載、建立索引、並從本地索引查詢資料。
   */
  const DataModule = (() => {
    // --- 預載與索引建立 ---
    const fetchAllBaseData = async (signal, onProgress) => {
        onProgress(0, '準備預載資料...');
        const fetchProducts = ApiModule.callApi('/planCodeController/query', {
            currentPage: 1,
            pageSize: ConfigModule.DEFAULT_QUERY_PARAMS.PAGE_SIZE_MASTER
        }, signal);

        const fetchChannels = ApiModule.callApi('/planCodeSaleDateController/query', {
            planCode: "", channel: "", saleEndDate: "", pageIndex: 1,
            size: ConfigModule.DEFAULT_QUERY_PARAMS.PAGE_SIZE_CHANNEL
        }, signal);

        onProgress(20, '正在獲取商品主檔與通路檔...');
        const [productRes, channelRes] = await Promise.all([fetchProducts, fetchChannels]);
        onProgress(60, '資料獲取完畢，建立本地索引...');

        const productMap = buildProductMap(productRes.records || []);
        onProgress(80, '商品索引完成...');
        
        const channelMap = buildChannelMap(channelRes.planCodeSaleDates?.records || []);
        onProgress(100, '通路索引完成...');

        StateModule.set({ productMap, channelMap });
    };

    const buildProductMap = (records) => {
        const map = new Map();
        const today = UtilsModule.formatToday();
        records.forEach(item => {
            map.set(item.planCode, {
    ...
    const buildChannelMap = (records) => {
        ...
        records.forEach(r => {
            ...
            map.get(planCode).push({
                ...
                status: UtilsModule.getSaleStatus(today, r.saleStartDate, r.saleEndDate)
            });
        });
        return map;
    };
    ...
    const queryLocalData = () => {
        ...
        const { productMap, channelMap } = state;
        let results = [];

        // 根據不同模式從 Map 中過濾資料
        switch (state.lastQuery.queryMode) {
            case ConfigModule.QUERY_MODES.PLAN_CODE:
                const codes = UtilsModule.splitInput(state.lastQuery.queryInput);
                codes.forEach(code => {
                    if (productMap.has(code)) {
                        results.push(productMap.get(code));
                    } else {
                        results.push({ planCode: code, _isErrorRow: true, _apiStatus: '查無此代號主檔' });
                    }
                });
                break;
            
            case ConfigModule.QUERY_MODES.PLAN_NAME:
                const nameKeyword = state.lastQuery.queryInput.toLowerCase();
                productMap.forEach(product => {
                    if (product.shortName.toLowerCase().includes(nameKeyword)) {
            ...
            case ConfigModule.QUERY_MODES.MASTER_CLASSIFIED:
                productMap.forEach(product => {
                    if (state.lastQuery.masterStatusSelection.has(product.mainStatus)) {
                        results.push(product);
                    }
                });
                break;
            ...
            case ConfigModule.QUERY_MODES.CHANNEL_CLASSIFIED:
                const targetChannels = Array.from(state.lastQuery.channelSelection);
                const targetStatus = state.lastQuery.channelStatusSelection;

                productMap.forEach(product => {
                    const productChannels = channelMap.get(product.planCode) || [];
                    const hasMatchingChannel = productChannels.some(ch => {
                        // 如果使用者有指定通路，則必須是指定通路之一
                        const channelMatch = targetChannels.length > 0 ? targetChannels.includes(ch.channel) : true;
                        if (!channelMatch) return false;
                        // 判斷銷售狀態是否符合
                        const isCurrentlyInSale = ch.status === ConfigModule.MASTER_STATUS_TYPES.IN_SALE;
                        return (targetStatus === '現售' && isCurrentlyInSale) || (targetStatus === '停售' && !isCurrentlyInSale);
                    });
    ...
    const enrichData = (data) => {
        const { channelMap } = StateModule.get();
        return data.map((item, index) => {
            ...
            const channels = channelMap.get(item.planCode) || [];
            const enrichedItem = {
                ...item,
                no: index + 1,
                polpln: '...', // POLPLN 永遠是按需載入
                channels: channels,
                _loadingDetails: true, // 代表 POLPLN 待載入
            };
            enrichedItem.specialReason = UtilsModule.checkSpecialStatus(enrichedItem);
            return enrichedItem;
        });
    };
    ...
    const getPolplnData = async (planCode, signal) => {
        ...
        const polplnRecords = (detail.records || []).map(r => extractPolpln(r.polpln)).filter(Boolean);
        if (polplnRecords.length === 0) {
            cacheDetail.set(planCode, "無資料");
            return "無資料";
        }
        const firstPolpln = polplnRecords[0];
        const result = polplnRecords.every(p => p === firstPolpln) ? firstPolpln : "-";
    ...
    const sortData = (data, key, asc) => {
      ...
      return [...data].sort((a, b) => {
        ...
        if (key.toLowerCase().includes('date')) {
          // 如果是，就去取用原始的日期資料 (例如 'rawSaleStartDate') 來排序，這樣才準確
  ...
  const ControllerModule = (() => {
    ...
    const initialize = async () => {
      ...
      if (storedToken) {
        ...
        if (await ApiModule.verifyToken(storedToken)) {
          ...
          await preloadDataAndShowQueryDialog();
        } else {
    ...
    const preloadDataAndShowQueryDialog = async () => {
        const controller = new AbortController();
        StateModule.set({ currentQueryController: controller, isPreloading: true });
        
        // 顯示一個特殊的預載入 Modal
        const preloadModalContent = [
            UIModule.createElement('div', { className: 'pct-modal-header', textContent: `正在初始化工具 (${StateModule.get().env})` }),
            UIModule.createElement('div', { className: 'pct-modal-body' })
        ];
        UIModule.Modal.show(preloadModalContent, (modal) => {
            modal.setAttribute('data-size', 'query');
            UIModule.Progress.show('準備中...');
        });

        try {
            await DataModule.fetchAllBaseData(controller.signal, UIModule.Progress.update);
            UIModule.Toast.show('本地索引建立完成!', 'success');
            StateModule.set({ isPreloading: false });
            showQueryDialog();
        } catch (error) {
            StateModule.set({ isPreloading: false });
            UIModule.Progress.hide();
            if (error.name !== 'AbortError') {
                UIModule.showError(`預載資料失敗: ${error.message}`, 'preload-error');
                UIModule.Toast.show('預載資料失敗，請檢查網路或Token', 'error', 5000);
            }
        } finally {
    ...
    const showTokenDialog = () => {
        ...
        const modalContent = [
            UIModule.createElement('div', { className: 'pct-modal-header', textContent: `設定 Token (${env})` }),
            UIModule.createElement('button', { className: 'pct-modal-close-btn', textContent: '×' }),
            UIModule.createElement('div', { className: 'pct-modal-body', children: [
                UIModule.createElement('div', { className: 'pct-form-group', children: [
                    UIModule.createElement('label', { htmlFor: 'pct-token-input', textContent: '請貼上您的 SSO-TOKEN 或 euisToken：' }),
                    UIModule.createElement('textarea', { id: 'pct-token-input', className: 'pct-input', rows: 4, placeholder: '(使用者在此貼上 Token...)' }),
                    UIModule.createElement('span', { id: 'token-error', className: 'pct-error' })
                ]})
            ]}),
            UIModule.createElement('div', { className: 'pct-modal-footer', children: [
                UIModule.createElement('div', { className: 'pct-modal-footer-left' }),
                UIModule.createElement('div', { className: 'pct-modal-footer-right', children: [
                    UIModule.createElement('button', { id: 'pct-skip-verification', className: 'pct-btn pct-btn-outline', textContent: '略過' }),
                    UIModule.createElement('button', { id: 'pct-verify-token', className: 'pct-btn', textContent: '驗證並初始化' })
                ]})
            ]})
        ];
        ...
        UIModule.Modal.show(modalContent, (modal) => {
            ...
            const handleVerifyToken = async () => {
                ...
                if (!token) { UIModule.showError('請輸入 Token', 'token-error'); return; }
                ...
                try {
                    ...
                    verifyBtn.textContent = '驗證並初始化';
                }
            };
            const handleSkipVerification = () => {
                const token = tokenInput.value.trim();
                StateModule.set({ token, tokenCheckEnabled: false });
                UIModule.Toast.show('已略過驗證，開始初始化', 'warning');
                preloadDataAndShowQueryDialog();
            };

            verifyBtn.onclick = handleVerifyToken;
            skipBtn.onclick = handleSkipVerification;
            tokenInput.addEventListener('keydown', (e) => { if (e.key === 'Enter' && e.ctrlKey) handleVerifyToken(); });
        });
    };
    ...
    const showQueryDialog = () => {
        ...
        // 建立 UI 元素
        const modalContent = [
            UIModule.createElement('div', { className: 'pct-modal-header', textContent: `選擇查詢條件 (${env})` }),
            UIModule.createElement('button', { className: 'pct-modal-close-btn', textContent: '×' }),
            UIModule.createElement('div', { className: 'pct-modal-body', children: [
                UIModule.createElement('div', { className: 'pct-form-group', children: [
                    UIModule.createElement('label', { textContent: '查詢模式:' }),
                    UIModule.createElement('div', { className: 'pct-mode-card-grid', children: [
                        UIModule.createElement('div', { className: 'pct-mode-card', textContent: '商品代號', dataset: { mode: ConfigModule.QUERY_MODES.PLAN_CODE } }),
                        UIModule.createElement('div', { className: 'pct-mode-card', textContent: '商品名稱', dataset: { mode: ConfigModule.QUERY_MODES.PLAN_NAME } }),
                        UIModule.createElement('div', { className: 'pct-mode-card', textContent: '商品銷售時間', dataset: { mode: ConfigModule.QUERY_MODES.MASTER_CLASSIFIED } }),
                        UIModule.createElement('div', { className: 'pct-mode-card', textContent: '通路銷售時間', dataset: { mode: ConfigModule.QUERY_MODES.CHANNEL_CLASSIFIED } })
                    ]}),
                    UIModule.createElement('div', { id: 'pct-dynamic-options', style: { display: 'none' } }),
                    UIModule.createElement('span', { id: 'query-error', className: 'pct-error' })
                ]})
            ]}),
            UIModule.createElement('div', { className: 'pct-modal-footer', children: [
                UIModule.createElement('div', { className: 'pct-modal-footer-left', children: [
                    UIModule.createElement('button', { id: 'pct-change-token', className: 'pct-btn pct-btn-outline', textContent: '修改 Token' })
                ]}),
                UIModule.createElement('div', { className: 'pct-modal-footer-right', children: [
                    UIModule.createElement('button', { id: 'pct-start-query', className: 'pct-btn', textContent: '開始查詢', disabled: true })
                ]})
            ]})
        ];
        ...
        UIModule.Modal.show(modalContent, (modal) => {
            ...
            const updateDynamicOptions = (mode) => {
                ...
                switch (mode) {
                    case ConfigModule.QUERY_MODES.PLAN_CODE:
                        contentElements = [
                            UIModule.createElement('label', { htmlFor: 'pct-query-input', textContent: '商品代碼：(多筆可用空格、逗號或換行分隔)' }),
                            UIModule.createElement('textarea', { id: 'pct-query-input', className: 'pct-input', rows: 4, value: lastQuery.queryInput || '' })
                        ];
                        break;
                    case ConfigModule.QUERY_MODES.PLAN_NAME:
                        contentElements = [
                            UIModule.createElement('label', { htmlFor: 'pct-query-input', textContent: '商品名稱關鍵字：' }),
                            UIModule.createElement('input', { type: 'text', id: 'pct-query-input', className: 'pct-input', value: lastQuery.queryInput || '' })
                        ];
                        break;
                    case ConfigModule.QUERY_MODES.MASTER_CLASSIFIED:
                        const statuses = Object.values(ConfigModule.MASTER_STATUS_TYPES);
                        contentElements = [
                            UIModule.createElement('label', { textContent: '主約銷售狀態：' }),
                            UIModule.createElement('div', { className: 'pct-sub-option-grid', children:
                                statuses.map(s => UIModule.createElement('div', {
                                    className: `pct-sub-option ${lastQuery.masterStatusSelection.has(s) ? 'selected' : ''}`,
                                    textContent: s,
                                    dataset: { status: s }
                                }))
                            })
                    ...
                    case ConfigModule.QUERY_MODES.CHANNEL_CLASSIFIED:
                        contentElements = [
                            UIModule.createElement('label', { textContent: '選擇通路：(可多選，不選代表全部)' }),
                            UIModule.createElement('div', { className: 'pct-channel-option-grid', children:
                                ConfigModule.FIELD_MAPS.CHANNELS.map(ch => UIModule.createElement('div', {
                                    className: `pct-channel-option ${lastQuery.channelSelection.has(ch) ? 'selected' : ''}`,
                                    textContent: ch,
                                    dataset: { channel: ch }
                                }))
                            }),
                            UIModule.createElement('label', { textContent: '銷售範圍：', style: { marginTop: '10px' }}),
                            UIModule.createElement('div', { className: 'pct-sub-option-grid', children:
                                ['現售', '停售'].map(range => UIModule.createElement('div', {
                                    className: `pct-sub-option ${lastQuery.channelStatusSelection === range ? 'selected' : ''}`,
                                    textContent: `${range}商品`,
                                    dataset: { range: range }
                                }))
                            })
                        ];
                ...
                if (contentElements.length > 0) {
                    contentElements.forEach(el => dynamicOptions.appendChild(el));
            ...
            const bindDynamicEvents = () => {
                document.querySelectorAll('.pct-sub-option[data-status]').forEach(option => option.onclick = () => {
                    option.classList.toggle('selected');
                    updateMasterStatusSelection();
                    checkCanStartQuery();
                });
                document.querySelectorAll('.pct-channel-option').forEach(option => option.onclick = () => {
                    option.classList.toggle('selected');
                    updateChannelSelection();
                    checkCanStartQuery();
                });
                document.querySelectorAll('.pct-sub-option[data-range]').forEach(option => option.onclick = () => {
                    const currentSelection = option.classList.contains('selected');
                    ...
                    if (!currentSelection) {
                        ...
                        StateModule.set({ channelStatusSelection: option.dataset.range });
                         ...
                         StateModule.set({ channelStatusSelection: '' });
                    }
                    checkCanStartQuery();
                });
                const input = document.getElementById('pct-query-input');
                if (input) {
                    input.addEventListener('input', EventModule.autoFormatInput);
                    input.addEventListener('input', checkCanStartQuery);
                }
            };
            
            const updateMasterStatusSelection = () => {
                const selected = new Set();
                document.querySelectorAll('.pct-sub-option[data-status].selected').forEach(o => selected.add(o.dataset.status));
                StateModule.set({ masterStatusSelection: selected });
            };
            const updateChannelSelection = () => {
                const selected = new Set();
                document.querySelectorAll('.pct-channel-option.selected').forEach(o => selected.add(o.dataset.channel));
            ...
            const checkCanStartQuery = () => {
                ...
                switch (state.queryMode) {
                    ...
                    case ConfigModule.QUERY_MODES.PLAN_NAME:
                        const input = document.getElementById('pct-query-input');
                        canStart = input && input.value.trim() !== '';
                        break;
                    case ConfigModule.QUERY_MODES.MASTER_CLASSIFIED:
                        canStart = state.masterStatusSelection.size > 0;
                        break;
                    case ConfigModule.QUERY_MODES.CHANNEL_CLASSIFIED:
                        canStart = state.channelStatusSelection !== '';
                        break;
                }
                startQueryBtn.disabled = !canStart;
            };

            modeCards.forEach(card => card.onclick = () => {
            ...
            // 【UX優化】還原上次查詢條件
            if (lastQuery.queryMode) {
                modal.querySelector(`.pct-mode-card[data-mode="${lastQuery.queryMode}"]`)?.click();
            }
    ...
    const handleStartQuery = () => {
        ...
        const input = document.getElementById('pct-query-input');
        if (input) {
            StateModule.set({ queryInput: input.value.trim() });
        }
        StateModule.backupQuery(); // 保存本次查詢條件
    ...
    const showResultsDialog = () => {
        const state = StateModule.get();
        const totalCount = state.currentResults.length;

        // 【UX強化】新增手動載入 POLPLN 按鈕
        const modalContent = [
            UIModule.createElement('div', { className: 'pct-modal-header', textContent: `查詢結果 (${state.env})` }),
            UIModule.createElement('button', { className: 'pct-modal-close-btn', textContent: '×' }),
            UIModule.createElement('div', { className: 'pct-modal-body', children: [
                UIModule.createElement('div', { style: { display: 'flex', flexDirection: 'column', height: '100%' }, children: [
                    UIModule.createElement('div', { style: { display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '10px', flexShrink: 0 }, children: [
                        UIModule.createElement('div', { style: { display: 'flex', gap: '8px', alignItems: 'center' }, children: [
                            UIModule.createElement('span', { style: { fontSize: '14px', color: '#666' }, textContent: '搜尋:' }),
                            UIModule.createElement('input', { type: 'text', id: 'pct-search-input', style: { padding: '4px 8px', border: '1px solid #ddd', borderRadius: '4px', width: '200px', fontSize: '13px' }})
                        ]}),
                        UIModule.createElement('button', { id: 'pct-load-all-details', className: 'pct-btn pct-btn-outline', textContent: '載入全部 POLPLN', style: { padding: '5px 12px', fontSize: '14px' }}),
                        UIModule.createElement('div', { id: 'pct-status-filters', style: { display: 'flex', gap: '5px' }})
                    ]}),
                    UIModule.createElement('div', { className: 'pct-table-wrap', children: [
                        UIModule.createElement('table', { className: 'pct-table', children: [
                            UIModule.createElement('thead', { children: [
                                UIModule.createElement('tr', { children: [
                                    UIModule.createElement('th', { dataset: { key: 'no' }, textContent: 'No' }),
                                    UIModule.createElement('th', { dataset: { key: 'planCode' }, textContent: '代號' }),
                                    UIModule.createElement('th', { dataset: { key: 'shortName' }, textContent: '名稱' }),
                                    UIModule.createElement('th', { dataset: { key: 'currency' }, textContent: '幣別' }),
                                    UIModule.createElement('th', { dataset: { key: 'unit' }, textContent: '單位' }),
                                    UIModule.createElement('th', { dataset: { key: 'coverageType' }, textContent: '類型' }),
                                    UIModule.createElement('th', { dataset: { key: 'saleStartDate' }, textContent: '主約銷售日' }),
                                    UIModule.createElement('th', { dataset: { key: 'saleEndDate' }, textContent: '主約停賣日' }),
                                    UIModule.createElement('th', { dataset: { key: 'mainStatus' }, textContent: '險種狀態' }),
                                    UIModule.createElement('th', { dataset: { key: 'polpln' }, textContent: '商品名稱(POLPLN)' }),
                                    UIModule.createElement('th', { textContent: '銷售通路' }),
                                ]})
                            ]}),
                            UIModule.createElement('tbody', { id: 'pct-table-body' })
                        ]})
                    ]})
                ]})
            ]}),
            UIModule.createElement('div', { className: 'pct-modal-footer', children: [
                UIModule.createElement('div', { className: 'pct-modal-footer-left', children: [
                     UIModule.createElement('button', { id: 'pct-toggle-view', className: 'pct-btn pct-btn-outline', textContent: '一頁顯示' }),
                     UIModule.createElement('button', { id: 'pct-filter-special', className: 'pct-btn pct-btn-outline', textContent: '篩選特殊' })
                ]}),
                 UIModule.createElement('div', { className: 'pct-modal-footer-center', children: [
                    UIModule.createElement('div', { className: 'pct-pagination', children: [
                        UIModule.createElement('button', { id: 'pct-prev-page', className: 'pct-btn pct-btn-outline', textContent: '◀' }),
                        UIModule.createElement('span', { id: 'pct-page-info', className: 'pct-pagination-info' }),
                        UIModule.createElement('button', { id: 'pct-next-page', className: 'pct-btn pct-btn-outline', textContent: '▶' })
                    ]})
                ]}),
                UIModule.createElement('div', { className: 'pct-modal-footer-right', children: [
                    UIModule.createElement('button', { id: 'pct-copy-all', className: 'pct-btn pct-btn-secondary', textContent: '一鍵複製' }),
                    UIModule.createElement('button', { id: 'pct-back-to-query', className: 'pct-btn', textContent: '重新查詢' })
                ]})
            ]})
        ];
        ...
        UIModule.Modal.show(modalContent, (modal) => {
            ...
            modal.classList.add('pct-view-results'); // 【UI修正】加上 class 以觸發 CSS 規則
    ...
    const setupResultsDialog = (modal) => {
        ...
        modal.addEventListener('click', (e) => {
            ...
            else if (target.matches('.clickable-cell') && planCode) {
    ...
    const rerenderTable = () => {
        ...
        let displayData = state.isFullView ? filteredData : filteredData.slice((state.pageNo - 1) * state.pageSize, state.pageNo * state.pageSize);
        
        const tableBody = document.getElementById('pct-table-body');
        if (tableBody) {
            tableBody.innerHTML = ''; // Clear
            displayData.forEach(item => tableBody.appendChild(renderTableRow(item)));
        }
    ...
    const renderTableRow = (item) => {
        const tr = UIModule.createElement('tr', {
            className: `${item.specialReason ? 'special-row' : ''}`,
            dataset: { plancode: item.planCode },
            title: item.specialReason ? UtilsModule.escapeHtml(item.specialReason) : ''
        });
        
        const renderStatusPill = (status) => `<span class="clickable-cell" style="display: inline-block; min-width: 80px; text-align: center; padding: 2px 6px; background: #f0f0f0; border-radius: 12px; font-size: 11px;">${status}</span>`;
        const renderPolplnCell = (item) => {
            if (item._loadingDetails) return UIModule.createElement('button', { className: 'load-details-btn', textContent: '載入' });
            if (item.polplnError) return UIModule.createElement('span', { className: 'load-details-error', textContent: '載入失敗', title: item.polplnError });
            return UIModule.createElement('span', { className: 'clickable-cell', textContent: UtilsModule.escapeHtml(item.polpln) });
        };
        const renderChannelsCell = (channels) => {
            if (!channels || channels.length === 0) return '-';
            const active = channels.filter(c => c.status === ConfigModule.MASTER_STATUS_TYPES.IN_SALE).sort((a,b) => a.channel.localeCompare(b.channel));
            const inactive = channels.filter(c => c.status !== ConfigModule.MASTER_STATUS_TYPES.IN_SALE).sort((a,b) => a.channel.localeCompare(b.channel));
            const activeHTML = active.map(c => `<span class="pct-channel-insale" title="${c.channel} (${c.status}): ${c.saleStartDate}-${c.saleEndDate}">${c.channel}</span>`).join(' ');
            const inactiveHTML = inactive.map(c => `<span class="pct-channel-offsale" title="${c.channel} (${c.status}): ${c.saleStartDate}-${c.saleEndDate}">${c.channel}</span>`).join(' ');
            return activeHTML + (active.length > 0 && inactive.length > 0 ? '<span class="pct-channel-separator">|</span>' : '') + inactiveHTML;
        };
        ...
        `;
        tr.children[9].appendChild(renderPolplnCell(item)); // Append POLPLN cell content
        return tr;
    };
    
    // --- Event Handlers for Results Dialog ---
    const handleToggleView = (btn) => {
        const state = StateModule.get();
        StateModule.set({ isFullView: !state.isFullView, pageNo: 1 });
        btn.textContent = state.isFullView ? '分頁顯示' : '一頁顯示';
        rerenderTable();
    };
    const handleFilterSpecial = (btn) => {
        const state = StateModule.get();
        StateModule.set({ filterSpecial: !state.filterSpecial, pageNo: 1 });
        btn.classList.toggle('pct-btn-primary', state.filterSpecial);
        rerenderTable();
    };
    const handlePageChange = (delta) => {
        ...
        const newPage = state.pageNo + delta;
        if (newPage >= 1 && newPage <= maxPage) {
            StateModule.set({ pageNo: newPage });
            rerenderTable();
        }
    };
    const handleSort = (th) => {
    ...
    // --- 按需載入邏輯 (POLPLN) ---
    const loadSingleDetail = async (planCode, btn) => {
        const item = StateModule.get().currentResults.find(d => d.planCode === planCode);
        ...
        const controller = new AbortController();
        try {
            const polpln = await DataModule.getPolplnData(planCode, controller.signal);
            item.polpln = polpln;
        } catch (error) {
            if (error.name !== 'AbortError') {
                item.polplnError = error.message;
                UIModule.Toast.show(`載入 POLPLN 失敗: ${error.message}`, 'error');
            }
        } finally {
    ...
    const loadAllDetailsInBatches = async () => {
        const state = StateModule.get();
        const itemsToLoad = state.currentResults.filter(item => item._loadingDetails && !item._isErrorRow);
        if (itemsToLoad.length === 0) {
            UIModule.Toast.show('所有 POLPLN 資料均已載入', 'info');
            return;
        }
        ...
        try {
            for (let i = 0; i < itemsToLoad.length; i += BATCH_SIZE) {
                ...
                const progress = ((i + batch.length) / itemsToLoad.length) * 100;
                UIModule.Progress.update(progress, `正在載入 ${i + batch.length}/${itemsToLoad.length}...`);

                await Promise.all(batch.map(async item => {
                    try {
                        const polpln = await DataModule.getPolplnData(item.planCode, controller.signal);
                        item.polpln = polpln;
                    } catch (err) {
                        item.polplnError = err.message;
                    } finally {
                        item._loadingDetails = false;
                    }
                }));
            ...
            UIModule.Progress.hide();
            UIModule.Toast.show('POLPLN 批次載入完成', 'success');
        } catch (error) {
            UIModule.Progress.hide();
            if (error.name !== 'AbortError') {
                console.error("POLPLN 批次載入失敗:", error);
                UIModule.Toast.show(`批次載入失敗: ${error.message}`, 'error');
            }
        } finally {
            StateModule.set({ currentQueryController: null });
        }
    ...
    const copyAllResults = () => {
        ...
        if (filteredData.length === 0) {
            UIModule.Toast.show('無資料可複製', 'warning'); return;
        }
        const headers = ['No', '代號', '名稱', '幣別', '單位', '類型', '主約銷售日', '主約停賣日', '險種狀態', '商品名稱(POLPLN)', '銷售通路'];
    ...
    try {
      ...
      document.addEventListener('pct:resultsShown', () => {
          setTimeout(() => {
              ...
              // 如果需要自動載入的項目過多，提示使用者手動載入
              if (itemsToLoad.length > 50) {
                  UIModule.Toast.show(`發現 ${itemsToLoad.length} 筆 POLPLN 待載入，建議手動點擊按鈕載入。`, 'info', 5000);
              } else if (itemsToLoad.length > 0) {
                  ControllerModule.loadAllDetailsInBatches();
              }
          }, 1000);
      }, { once: true });
