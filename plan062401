// PLANNN.js - 商品查詢小工具 (v0.2)
// 更新：使用正確 UAT/PROD API 端點，支援 sso-token/euisToken，textarea rows=2，拖曳視窗，yyyymmdd 日期
...
(function() {
    ...
    const AppConfig = (function() {
        ...
        return { STYLE_ID, API_ENDPOINTS, API_PATHS, QUERY_MODES, DEFAULT_QUERY_PARAMS, FIELD_MAPS };
    })();
    ...
    const Utils = (function() {
        ...
        return { getToday, formatDate, checkSpecialStatus, splitInput, copyToClipboard };
    })();
    ...
    const APIService = (function() {
        ...
        return { setToken, verifyToken, request };
    })();
    ...
    const DataProcessor = (function(AppConfig, Utils) {
        ...
        function getCachedData() {
            return { masterDataCache, channelDataCache };
        }

        return { resetData, processAllDataForTable, getCachedData };
    })(AppConfig, Utils);
    ...
    const AppCore = (function(AppConfig, UIManager, APIService, DataProcessor, Utils) {
        ...
        function renderTableHTML(data) {
            const headers = [
                ...
                { key: 'planName', label: '商品名稱' },
                { key: 'currency', label: '幣別' },
                { key: 'unit', label: '單位' },
                { key: 'planType', label: '類型' },
                { key: 'saleStartDate', label: '銷售起日' },
                { key: 'saleEndDate', label: '銷售止日' },
                { key: 'status', label: '主約狀態' },
                { key: 'polPln', label: 'POLPLN' },
                { key: 'channelInfo', label: '通路資訊' },
                { key: 'special', label: '特殊狀態' }
            ];
            ...
            UIManager.showModal({
                ...
                onOpen: (modalElement) => {
                    ...
                    detailBtn.onclick = async () => {
                        ...
                        try {
                            const { masterDataCache } = DataProcessor.getCachedData();
