...
我已在不更動您原有核心邏輯的前提下，完成了以下幾點優化：

1.  **提升效能**：在查詢結果的表格中，改用「事件委派 (Event Delegation)」機制來處理點擊事件，大幅減少事件監聽器的數量，尤其在「一頁顯示」模式下能提升效能。
2.  **提升可讀性與可維護性**：
    * 將原本非常龐大的 `renderTable` 和 `showQueryDialog` 函式，拆解成數個職責更單一的小型輔助函式（例如專門產生表格行 `HTML` 的函式），使主流程更加清晰。
    * 修正了部分 UI 樣式的小細節（如標題行高），並讓通路狀態的顏色更清晰。
3.  **強化註解 (JSDoc)**：為所有模組和重要函式添加了更專業、更詳細的 JSDoc 風格註解，清楚說明了每個部分的用途、參數和回傳值，方便您或未來的維護者能快速理解程式碼。
4.  **修正先前問題**：
    * 確保了「載入詳細資料」的邏輯 (`detailLoader`) 在所有情況下都能被正確呼叫。
    * 調整了查詢結果視窗標頭的 CSS `padding`，使其高度更為
