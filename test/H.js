javascript:(function() {
  ...
  /**
   * ===================================================================
   * 商品查詢小工具 v9.0.0 (UI還原 & 功能整合最終版)
   *
   * - [重大修正] UI/CSS 完全還原至您提供的 v6.0.0 版本風格。
   * - [重大修正] 徹底解決查詢結果頁面破版、欄位超出外框的問題。
   * - [功能整合] 包含所有新功能：進度條、中止按鈕、主檔分類查詢、
   * 前端篩選器、手動觸發查詢等 15 項優化。
   * ===================================================================
   */
  ...
  const ControllerModule = (() => {
    ...
    const showTokenDialog = () => {
      ...
      UIModule.Modal.show(`
        <div class="pct-modal-header"><span>商品查詢工具 (${env})</span><button class="pct-modal-close-btn">&times;</button></div>
      ...
      `, modal => {
        ...
        modal.querySelector('#pct-token-skip').onclick = () => { const val = tokenInput.value.trim(); if (val) { StateModule.set({ token: val }); localStorage.setItem('SSO-TOKEN', val); } else { StateModule.set({ token: '' }); } UIModule.Toast.show('已略過 Token 驗證', 'warning'); StateModule.set({ tokenCheckEnabled: false }); showQueryDialog(); };
      });
    };

    const showQueryDialog = () => {
        const modeLabel = m => ({[ConfigModule.QUERY_MODES.PLAN_CODE]:'商品代號', [ConfigModule.QUERY_MODES.PLAN_NAME]:'商品名稱', [ConfigModule.QUERY_MODES.MASTER_CLASSIFIED]:'主檔分類查詢', [ConfigModule.QUERY_MODES.CHANNEL_CLASSIFIED]:'通路分類查詢'}[m] || m);
        UIModule.Modal.show(`
            <div class="pct-modal-header"><span>查詢條件設定</span><button class="pct-modal-close-btn">&times;</button></div>
            ...
            <div class="pct-modal-footer"><div class="pct-modal-footer-left"><button class="pct-btn pct-btn-secondary" id="pct-query-clear">清除</button></div><div class="pct-modal-footer-right"><button class="pct-btn" id="pct-query-ok">開始查詢</button></div></div>
        `, modal => {
            modal.setAttribute('data-size', 'query');
            ...
            const updateUI = () => {
                ...
                switch (localState.mode) {
                    ...
                    case ConfigModule.QUERY_MODES.MASTER_CLASSIFIED: content = `<div class="pct-form-group"><label class="pct-label">主檔狀態篩選 (可複選)：</label><div class="pct-sub-option-grid">${Object.values(ConfigModule.MASTER_STATUS_TYPES).map(s=>`<div class="pct-sub-option" data-type="masterStatus" data-value="${s}">${s}</div>`).join('')}</div><p style="font-size:12px;color:#666;margin-top:15px;">ⓘ 將掃描所有主檔資料，執行時間可能較長。</p></div>`; break;
            ...
            modal.querySelector('#pct-query-clear').onclick = () => { localState = { mode: '', input: '', masterStatus: new Set(), channelStatus: '', channels: new Set() }; updateUI(); };
            
            okBtn.onclick = () => {
                UIModule.hideError('pct-query-err');
                if (localState.mode === ConfigModule.QUERY_MODES.MASTER_CLASSIFIED) {
                    if (localState.masterStatus.size === 0) { UIModule.showError('請至少選擇一個主檔狀態', 'pct-query-err'); return; }
