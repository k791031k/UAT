javascript:(function() {
  ...
  /**
   * ===================================================================
   * 商品查詢小工具 v18.4.0 (版面修正版)
   *
   * @version 18.4.0
   * @description
   * - [修正] 根據使用者提供的設計範例，徹底解決「選擇查詢條件」對話框的版面破壞問題。
   * - [修正] 移除所有不合理的固定高度與最小高度限制，讓介面佈局由內容自然決定，消除多餘空白。
   * - [修正] 確保對話框內不再出現垂直捲動條，所有選項皆能完整顯示。
   * - [優化] 微調選項區塊的間距，使其更為緊湊、美觀。
   * - [不變] 所有 JavaScript 核心功能、查詢邏輯、按鈕行為皆與先前版本完全一致。
   * ===================================================================
   */
  ...
  const ConfigModule = Object.freeze({
    ...
    VERSION: '18.4.0-Layout-Fix-Final',
    QUERY_MODES: {
      PLAN_CODE: 'planCode',
  ...
  const StateModule = (() => {
    const state = {
      ...
      apiBase: '', token: '', tokenCheckEnabled: true, tokenWasManuallySet: false,
  ...
  const UIModule = (() => {
    const injectStyle = () => {
      ...
      s.textContent = `
        ...
        .pct-modal-body{padding:16px 20px 8px 20px;flex-grow:1;}
        ...
        .pct-label { display: block; margin-bottom: 8px; font-weight: 500; color: var(--text-color-light); }
        #pct-dynamic-query-content { margin-top: 15px; }
      ...
      const groupedByPlanCode = rawData.reduce((acc, cur) => {
        if (!acc[cur.planCode]) {
          acc[cur.planCode] = {
            ...
            _isErrorRow: false, _loadingProduct: true, _loadingDetails: true
          };
        }
  ...
  const ControllerModule = (() => {
    ...
    const initialize = async () => {
      ...
      if (storedToken) {
        ...
        UIModule.Toast.show('正在自動驗證已儲存的 Token...', 'info', 2000);
        if (await ApiModule.verifyToken(storedToken)) {
          UIModule.Toast.show('Token 驗證成功', 'success');
          showQueryDialog();
        } else {
          UIModule.Toast.show('自動驗證失敗，請重新輸入或略過', 'warning');
    ...
    const showTokenDialog = () => {
      ...
      UIModule.Modal.show(html, modal => {
        ...
        const handleVerify = async () => {
            ...
            StateModule.set({ token: val, tokenCheckEnabled: true, tokenWasManuallySet: true });
        ...
        const handleSkip = () => {
            const val = tokenInput.value.trim();
            StateModule.set({ token: val, tokenCheckEnabled: false, tokenWasManuallySet: true });
            if (val) {
    ...
    const showQueryDialog = () => {
        const { env, tokenWasManuallySet } = StateModule.get();
        ...
        const footerLeftHTML = tokenWasManuallySet ? `<button class="pct-btn pct-btn-secondary" id="pct-back-to-token">返回修改 Token</button>` : '';
        ...
        UIModule.Modal.show(html, modal => {
            ...
            const updateDynamicContent = () => {
                ...
                switch (localState.mode) {
                    ...
                    case ConfigModule.QUERY_MODES.MASTER_CLASSIFIED: content = `<div class="pct-form-group"><label class="pct-label">商品銷售時間：</label><div class="pct-sub-option-grid">${Object.values(ConfigModule.MASTER_STATUS_TYPES).map(s=>`<div class="pct-sub-option" data-type="masterStatus" data-value="${s}">${s}</div>`).join('')}</div></div>`; break;
            ...
            const handleQueryClick = () => {
                ...
                if (!localState.mode) return;

                if (localState.mode === ConfigModule.QUERY_MODES.MASTER_CLASSIFIED) {
                    if (localState.masterStatus.size === 0) {
                        UIModule.showError('請至少選擇一個狀態', 'pct-query-err');
                        return;
                    }
                    if (localState.masterStatus.size === Object.keys(ConfigModule.MASTER_STATUS_TYPES).length) {
                        UIModule.Toast.confirm('您已選擇全部狀態，此操作可能耗時較長，確定要繼續嗎？', (confirmed) => {
                            if (confirmed) {
                                proceedWithQuery();
                            }
                        });
            ...
            modal.querySelector('#pct-mode-wrap').addEventListener('click', e => {
                ...
                if (card) { localState.mode = card.dataset.mode; updateDynamicContent(); }
            });
    ...
    const rerenderTable = () => {
        ...
        const dataForDisplayAndCopy = state.searchKeyword.trim()
            ? filteredData.filter(r => Object.values(r).some(v => String(v).toLowerCase().includes(state.searchKeyword.toLowerCase())))
            : filteredData;
            
        const sortedData = DataModule.sortData(dataForDisplayAndCopy, state.sortKey, state.sortAsc);
        ...
        const tableBodyHTML = pageData.map((r, index) => {
            ...
            const rowClass = `${r.specialReason ? 'special-row' : ''} ${r._loadingProduct || r._loadingDetails ? 'clickable-row' : ''}`.trim();
            ...
            return `
                <tr data-plan-code="${r.planCode}" class="${rowClass}" ${specialAttr}>
                    <td class="clickable-cell" data-raw="${rowNum}">${rowNum}</td>
                    ...
                    <td class="clickable-cell" data-raw="${(r.channels||[]).map(c=>`${c.channel}:${c.saleEndDate}`).join('; ')}">${getChannelHTML(r.channels)}</td>
                </tr>`;
        }).join('');

        const modalHTML = `
            <div class="pct-modal-header"><span>查詢結果 (${state.env})</span><button class="pct-modal-close-btn">&times;</button></div>
            <div class="pct-modal-body" style="overflow-y: auto;">
            ...
            <div class="pct-modal-footer">
                ...
                <div class="pct-modal-footer-right"><button class="pct-btn pct-btn-secondary" id="pct-table-detail">查詢資料</button><button class="pct-btn pct-btn-success" id="pct-table-copy">一鍵複製</button><button class="pct-btn" id="pct-table-requery">重新查詢</button></div>
            </div>`;
        ...
        UIModule.Modal.show(modalHTML, modal => {
            ...
            if (table) {
                table.addEventListener('click', e => {
                    ...
                    if (th) {
                        ...
                        if (key) { StateModule.set({ sortAsc: state.sortKey === key ? !state.sortAsc : true, sortKey: key, pageNo: 1 }); rerenderTable(); }
                    } else if (td) {
            ...
            modal.querySelector('#pct-table-copy').onclick = () => {
                ...
                const copyText = `${headers.map(h=>h.label).join('\t')}\n` + dataToCopy.map((r, i) => [i+1, r.planCode, r.shortName, r.currency, r.unit, r.coverageType, r.saleStartDate, r.saleEndDate, r.mainStatus, r.polpln, (r.channels||[]).map(c=>`${c.channel}:${c.saleEndDate}(${c.status})`).join(' / ')].join('\t')).join('\n');
            ...
            modal.querySelector('#pct-table-detail').onclick = () => {
                const itemsToLoad = state.allProcessedData.filter(item => !item._isErrorRow && (item._loadingDetails || item._loadingProduct));
                if (itemsToLoad.length > 0) {
                    const controller = new AbortController();
                    const detailLoader = () => Promise.all(itemsToLoad.map(item => {
                        if (item._loadingProduct) {
                            return DataModule.loadProductForSingleRow(item, controller.signal);
                        }
                        return DataModule.loadDetailsForSingleRow(item, controller.signal);
                    }));
                    executeQuery(detailLoader);
                } else {
                    UIModule.Toast.show('所有詳細資料均已載入完畢', 'info');
                }
            };
