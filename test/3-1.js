        ...
        `, modal => {
            ...
            okBtn.onclick = () => {
                ...
                if (localState.mode === ConfigModule.QUERY_MODES.MASTER_CLASSIFIED) {
                    ...
                    if (localState.masterStatus.size === Object.keys(ConfigModule.MASTER_STATUS_TYPES).length) { UIModule.Toast.confirm('將掃描全部資料，確定嗎？', (confirmed) => { if (confirmed) { StateModule.set({ queryMode: localState.mode, masterStatusSelection: localState.masterStatus }); executeQuery(); } }); return; }
                }
