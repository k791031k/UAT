javascript:(function() {
  ...
  /**
   * ===================================================================
   * 商品查詢小工具 v10.0.1 (最終修正完美版)
   *
   * - [最終修正] 將搜尋防抖延遲從 800ms 調整為您指定的 1000ms。
  ...
  const ConfigModule = Object.freeze({
    ...
    DEBOUNCE_DELAY: { SEARCH: 1000 }, // 修正：調整為 1000ms
  ...
  const UIModule = (() => {
    const injectStyle = () => {
      ...
      s.textContent = `
        ...
        .pct-mode-card.pct-status-card { color: white; border: none; }
        .pct-mode-card.pct-status-card.selected { border: 2px solid var(--surface-color); box-shadow: 0 0 0 2px var(--primary-dark-color); }
